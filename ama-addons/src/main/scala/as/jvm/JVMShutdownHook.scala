...
class JVMShutdownHook(receiver: ActorRef, message: AnyRef, keepHookForTimeInMs: Int) extends Thread {
  override def run = {
    try {
      println(s"${getClass.getSimpleName}: JVM shutdown signal detected! Will keep shutdown hook for ${keepHookForTimeInMs}ms while system will be shutting down...")
      receiver ! message
    } finally {
      Thread.sleep(keepHookForTimeInMs)
      println(s"${getClass.getSimpleName}: ${keepHookForTimeInMs}ms is over, releasing shutdown hook, bye!")
    }
  }
}
