...
class JVMSystemExitDaemon(delayInMs: Int, exitCode: Int) extends Thread {
  ...
  override def run = {
    println(s"${getClass.getSimpleName}: will shut down JVM in ${delayInMs}ms with exit code $exitCode")
    Thread.sleep(delayInMs)
    println(s"${getClass.getSimpleName}: ${delayInMs}ms is over, shutting down JVM with exit code $exitCode.")
